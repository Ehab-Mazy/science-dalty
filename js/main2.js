...
function loadImportedTests() {
    ...
    console.log(`✅ تم تحميل ${Object.keys(importedTests).length} اختبار مستورد`);
}

// دالة لعرض الاختبارات المستوردة
function displayImportedTests() {
    const container = document.getElementById('importedTestsList');
    if (!container) return;
    ...
    Object.values(importedTests).forEach(test => {
        const testHTML = `
            <div class="imported-test-item" style="margin: 10px 0; padding: 15px; background: #f8f9fa; border-radius: 10px; border-right: 4px solid #667eea;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <strong>${test.title}</strong>
                        <div style="font-size: 14px; color: #666;">
                            ${test.questions.length} سؤال ⏱️ ${test.time} دقيقة
                        </div>
                    </div>
...
// دالة لتحديث قائمة الاختبارات يدوياً
function refreshImportedTests() {
    loadImportedTests();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // تحميل وعرض الاختبارات المستوردة
    loadImportedTests();
    displayImportedTests();
    ...
    // نظام التحكم بالإدارة
let adminMode = false;

// دالة لتفعيل وضع الإدارة (يمكن استدعاؤها من console)
function enableAdminMode() {
    adminMode = true;
    document.querySelectorAll('.import-fab').forEach(btn => {
        btn.classList.add('admin-mode');
    });
    console.log('🔧 وضع الإدارة مفعل');
}

// دالة لتعطيل وضع الإدارة
function disableAdminMode() {
    adminMode = false;
    document.querySelectorAll('.import-fab').forEach(btn => {
        btn.classList.remove('admin-mode');
    });
    console.log('🔧 وضع الإدارة معطل');
}

// للاستخدام: افتح console المتصفح وأكتب enableAdminMode()
